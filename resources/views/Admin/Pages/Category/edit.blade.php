...
<div class="content-wrapper" style="margin-top: 50px">
    ...
    <div class="d-flex justify-content-between align-items-center skin-blue-style shadow text-dark p-5">
        <div>
            <h3 class="p-0 m-0">Sửa danh mục</h3>
            <p></p>
        </div>
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb m-0 bg-transparent text-dark">
                  ...
                  <li class="breadcrumb-item text-dark active" aria-current="page">Sửa danh mục</li>
                </ol>
              </nav>
        </div>
    </div>
    ...
    <section class="container-fluid">
      ...
      <!-- Default box -->
        <div class="my-5">
            <form action="{{ route('category.update', $category->id) }}" method="post" class="p-4 col-8 mx-auto shadow border border-0">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="name_inp">Tên danh mục</label>
                    <input type="text" class="form-control" name="name" id="name_inp"  value="{{ old("name") ?? $category->name }}" oninput="ChangeToSlug(this)" >
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input type="radio" class="mx-2" name="status" id="show" value="0" {{ old("status") ?? $category->status == '0' ? "checked" : "" }} checked>
                    <label for="hide" class="text-secondary">Ẩn</label>
                    <input type="radio" class="mx-2" name="status" id="hide" value="1" {{ old("status") ?? $category->status == '1' ? "checked" : "" }}>
                    @error('status')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                ...
                @else
                    <div class="form-group d-none">
                        <select class="" name="parent_id" id="parent_id" class="form-control">
